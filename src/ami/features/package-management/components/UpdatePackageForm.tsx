...
const UpdatePackageForm = () => {
	...
	const { data: pkg, isLoading: isPackageFetching } = useGetPackageByIdQuery(
		id!
	);
	...
	useEffect(() => {
		if (pkg && !hasInitialized.current) {
			...
			form.reset({
				...
				image: pkg.image ?? "",
				custom_duration_minutes: pkg.custom_duration_minutes ?? undefined,
			});
		}
	}, [pkg]);
	...
	if (isServicesFetching || isPackageFetching) {
		return (
					...
					<form onSubmit={form.handleSubmit(onSubmit)}>
											...
											{showAddService && (
												<div className="p-4 space-y-4 border-[1px] border-border rounded-lg bg-white">
													...
													<Button
														...
														disabled={
															!selectedServiceId || isUpdatePackageLoading
														}
													>
														Add to Package
													</Button>
						...
						<FormCard.Footer className="flex justify-end gap-2">
							<Button
								variant="secondary"
								type="button"
								disabled={!form.formState.isDirty || isUpdatePackageLoading}
								onClick={(e) => {
									e.stopPropagation();
									e.preventDefault();
									form.reset();
