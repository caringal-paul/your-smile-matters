...
import {
	...
	ApproveTransactionRequestSchema,
} from "../utils/schema/approve-transaction-request.ami.schema";
import {
...
} from "./TransactionForApprovalFormModals";
import { useGetTransactionForApprovalById } from "../queries/getTransactionForApprovalById.ami.query";
import { useRejectTransactionMutation } from "../queries/rejectTransaction.ami.query";
import { useApproveRefundTransactionRequestMutation } from "../queries/approveRefundTransactionRequest.ami.mutation";
import { useRejectRefundTransactionRequestMutation } from "../queries/rejectRefundTransactionRequest.ami.mutation";
...
// Transaction Action Modal - consistent with BookingActionModal
const TransactionActionModal: React.FC<TransactionActionModalProps> = ({
	...
	const config = {
		Approve: {
			...
			dialogTitle: "Approve Refund Request",
			dialogDesc:
				"Approving this refund will mark the transaction as refunded and notify the customer. This action cannot be undone. Are you sure?",
		...
		Reject: {
			...
			dialogTitle: "Reject Refund Request",
			dialogDesc:
				"Rejecting this refund request will notify the customer with the reason provided. Are you sure you want to proceed?",
...
const ViewTransactionForApproval: React.FC = () => {
	const { id } = useParams<{ id: string }>();
	const { data: request, isLoading } = useGetTransactionForApprovalById(id!);
	...
	const { mutateAsync: approveRefundMutation, isPending: isApprovingRefund } =
		useApproveRefundTransactionRequestMutation();
	const { mutateAsync: rejectRefundMutation, isPending: isRejectingRefund } =
		useRejectRefundTransactionRequestMutation();

	const approvalForm = useForm<ApproveTransactionRequestSchema>({
		resolver: zodResolver(approveTransactionRequestSchema),
		mode: "onChange",
		defaultValues: {
			admin_notes: "",
		},
	...
	const rejectionForm = useForm<RejectTransactionRequestSchema>({
		resolver: zodResolver(rejectTransactionRequestSchema),
		mode: "onChange",
		defaultValues: {
			rejection_reason: "",
			admin_notes: "",
	...
	}, [request?.status, openModal, isApprovingRefund, isRejectingRefund]);
			...
			<div className="pb-8 grid grid-cols-1 xl:grid-cols-3 gap-4">
				...
				<FormCard className="col-span-1 xl:col-span-3 w-full h-full">
					...
					<div className="flex flex-col w-full gap-4 p-4">
						<FormCard.Field>
							<FormCard.Label>Associated Transaction:</FormCard.Label>
							<Label className="font-normal text-2xs">
								{request.transaction_id?.transaction_reference || "-"}
							</Label>
						</FormCard.Field>
				...
				<FormCard className="col-span-1 xl:col-span-3 w-full h-full">
					...
					<div className="flex flex-col w-full gap-4 p-4">
						<FormCard.Field>
							<FormCard.Label>Transaction Reference:</FormCard.Label>
							<Label className="font-normal text-2xs">
								{request.transaction_id?.transaction_reference || "-"}
							</Label>
						</FormCard.Field>
						...
						<FormCard.Field>
							...
							<Label className="font-medium text-sm text-primary">
								{formatCurrency(request.transaction_id?.amount || 0)}
							</Label>
						</FormCard.Field>
						...
						<FormCard.Field>
							...
							<Label className="font-normal text-2xs">
								{request.transaction_id?.payment_method || "-"}
							</Label>
						</FormCard.Field>

						{request.transaction_id?.created_at && (
							<FormCard.Field>
								<FormCard.Label>Payment Date:</FormCard.Label>
								<Label className="font-normal text-2xs">
									{formatToNormalDate(request.transaction_id.created_at)}
								</Label>
							</FormCard.Field>
						)}

						<FormCard.Field>
							<FormCard.Label>Transaction Status:</FormCard.Label>
							<Badge
								className={cn(
									"text-2xs w-fit",
									REQUEST_STATUS_COLORS[
										request.transaction_id?.status as RequestStatus
									]
								)}
							>
				...
				<FormCard className="col-span-1 xl:col-span-3 w-full h-full">
					...
					<div className="flex flex-col w-full gap-4 p-4">
						...
						<FormCard.Field>
							<FormCard.Label>Booking Date:</FormCard.Label>
							<Label className="font-normal text-2xs">
				...
				<FormCard className="col-span-1 xl:col-span-3 w-full h-full bg-amber-50/30">
					<FormCard.Title className="flex flex-row items-center gap-2">
					...
					<div className="flex flex-col w-full gap-4 p-4">
						<FormCard.Field>
							<FormCard.Label>Refund Amount:</FormCard.Label>
							<Label className="font-bold text-lg text-amber-700">
								{formatCurrency(request.transaction_id?.amount || 0)}
							</Label>
						</FormCard.Field>

						<FormCard.Field>
							<FormCard.Label>Reason for Refund:</FormCard.Label>
...
export default ViewTransactionForApproval;
