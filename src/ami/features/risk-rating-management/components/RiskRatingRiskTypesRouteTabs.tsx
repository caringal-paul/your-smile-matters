...
import { RISK_RATING_RISK_TYPES_TAB_ROUTES } from "../constants/risk-rating-risk-types.constants";

const RiskRatingRiskTypesRouteTabs = () => {
	const navigate = useNavigate();

	const [isSelectPopoverOpen, setIsSelectPopoverOpen] = useState(false);
	const [tab, setTab] = useState("Risk Types");
		...
		<div>
			<div className="flex">
				<Tabs
					...
					defaultValue="risk-types"
				>
					<TabsList className="my-5 xl:my-4">
						{RISK_RATING_RISK_TYPES_TAB_ROUTES.map((route: TabRoute) => {
							return (
								<TabsTrigger
									key={route.id}
									value={route.value}
...
export default RiskRatingRiskTypesRouteTabs;
