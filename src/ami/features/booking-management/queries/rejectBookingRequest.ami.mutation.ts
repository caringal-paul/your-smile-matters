...
import { RejectBookingRequestSchema } from "../utils/schemas/reject-booking-request.ami.schema";

export const useRejectBookingRequestMutation = () => {
	return useMutation({
		mutationFn: async ({
			id,
			payload,
			...
			id: string;
			payload: RejectBookingRequestSchema;
		}) => {
			const res: BaseResponseDto<BookingRequestResponse> =
				await amiBookingRequestsApi.reject(id, payload);
		...
		onSuccess: (data) => {
			...
			toast.success("Reschedule booking approved!");
