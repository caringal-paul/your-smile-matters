...
import { INVESTMENT_LISTING_ROUTES } from "../constants/investment-listing.constants";

const InvestmentListingRouteTabs = () => {
	const navigate = useNavigate();
	const location = useLocation();

	const currentTab = INVESTMENT_LISTING_ROUTES.find((route) =>
			...
			<div className="flex">
				...
				>
					<TabsList className="my-5 xl:my-4">
						{INVESTMENT_LISTING_ROUTES.map((route: TabRoute) => {
							return (
								<TabsTrigger
									key={route.id}
									value={route.value}
...
export default InvestmentListingRouteTabs;
