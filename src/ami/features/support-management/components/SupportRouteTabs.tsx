...
import { SUPPORT_ROUTES } from "../constants/support-constants";

const SupportRouteTabs = () => {
	const navigate = useNavigate();

	const [isSelectPopoverOpen, setIsSelectPopoverOpen] = useState(false);
	const [tab, setTab] = useState("Frequently Asked Questions");

	return (
		<div>
			<div className="flex pt-4">
				<Tabs
					className="w-full hidden sm:flex mt-16 xl:mt-0"
					defaultValue="faq"
				>
					<TabsList className="my-5 xl:my-4">
						{SUPPORT_ROUTES.map((route: TabRoute) => {
							return (
								<TabsTrigger
									key={route.id}
									value={route.value}
			...
			<div>
				...
				>
					<PopoverTrigger asChild>
						<Button
							className={cn(
								"mt-16 w-full justify-between shadow-none transition-none sm:hidden bg-transparent hover:bg-transparent text-primary hover:text-text-primary border-b-2 rounded-none border-primary"
							)}
						>
...
export default SupportRouteTabs;
