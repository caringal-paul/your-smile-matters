...
import { PENDING_TRADES_ROUTES } from "../../constants/portfolio.constants";

const PendingTradeRouteTabs = () => {
	const navigate = useNavigate();

	const [isSelectPopoverOpen, setIsSelectPopoverOpen] = useState(false);
	const [tab, setTab] = useState("Pending Trades");
		...
		<div>
			<div className="flex">
				<Tabs
					...
					defaultValue="pending-trades"
				>
					<TabsList className="my-5 xl:my-4">
						{PENDING_TRADES_ROUTES.map((route: TabRoute) => {
							return (
								<TabsTrigger
									key={route.id}
									value={route.value}
...
export default PendingTradeRouteTabs;
